...
// Load config from vex_config (isolated by tenant_id)
async function loadVexConfig() {
    try {
        const { data } = await supabase
            .from("vex_config")
            .select("key,value")
            .eq("tenant_id", global.tenantId);
        
        data?.forEach(i => { 
            if (i.key && i.value) process.env[i.key] = i.value; 
        });
...
async function syncSessionToCloud(creds) {
    try {
        ...
        await supabase.from("vex_session").upsert({
            id: global.tenantId, // Isolated ID
            data: base64,
...
• Tenant: ${global.tenantId}`;
    ...
    sock.ev.on("messages.upsert", async ({ messages }) => {
        ...
        for (const o of observers) {
            try {
                if (!o.trigger || o.trigger(m)) await o.onMessage(m, sock, { supabase, cache, clientId: global.clientId });
            } catch (e) {}
        }
