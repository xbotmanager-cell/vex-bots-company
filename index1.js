...
// ================= UNIVERSAL SUPABASE WRAPPER (LAZY CLONING & ISOLATION) =================
...
supabase.from = (table) => {
    ...
    const ensureDataExists = async () => {
        try {
            if (t === "vex_default") return;
            const { count } = await origFrom(table).select('*', { count: 'exact', head: true }).eq('tenant_id', t);
            if (count === 0) {
                const { data: templates } = await origFrom(table).select('*').eq('tenant_id', 'vex_default');
                if (templates && templates.length > 0) {
                    const newRows = templates.map(row => {
                        const { id, created_at, ...rest } = row;
                        return { ...rest, tenant_id: t, client_id: t };
                    });
                    await origFrom(table).insert(newRows);
                }
            }
        } catch (e) {}
    };
    ...
    const wrapFilter = (fn) => (...args) => fn(...args).eq("tenant_id", t);
    ["select", "update", "delete"].forEach(m => { 
        if (q[m]) q[m] = wrapFilter(q[m].bind(q)); 
    });

    const ins = q.insert.bind(q);
    q.insert = (d, o) => ins(Array.isArray(d) ? d.map(x => ({ ...x, tenant_id: t })) : { ...d, tenant_id: t }, o);
...
async function syncSessionToCloud(creds) {
    try {
        ...
        await origFrom("vex_session").upsert({ id: global.tenantId, data: base64, tenant_id: global.tenantId }, { onConflict: 'id' });
    } catch (e) {}
}
...
async function startBot() {
    ...
    sock.ev.on("connection.update", async ({ connection, lastDisconnect, qr }) => {
        ...
        if (connection === "open") {
            ...
            try { await sock.sendMessage(sock.user.id, { image: { url: BOT_IMAGE }, caption: msg }); } catch (e) {}
        }
    });
    ...
    sock.ev.on("messages.upsert", async ({ messages }) => {
        ...
        for (const o of observers) {
            try { if (!o.trigger || o.trigger(m)) await o.onMessage(m, sock, { supabase, cache, clientId: global.clientId }); } catch (e) {}
        }
        try {
...
io.on("connection", socket => {
    socket.on("request_pair", async phone => {
        try {
            ...
            socket.emit("pairing_code", await sock.requestPairingCode(num));
        } catch (e) { socket.emit("pairing_code", "ERROR"); }
    });
    socket.on("get_stats", () => {
        const mins = Math.floor((Date.now() - START_TIME) / 60000);
        socket.emit("stats", { uptime: `${Math.floor(mins / 60)}h ${mins % 60}m` });
    });
});

server.listen(PORT, () => { console.log(`VEX running on ${PORT}`); startBot(); });
