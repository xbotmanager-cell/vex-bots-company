...
// WRAP SUPABASE - USE client_id (NOT tenant_id) FOR COMPATIBILITY WITH ALL PLUGINS
const origFrom = supabase.from.bind(supabase);
supabase.from = (table) => {
    const q = origFrom(table);
    if (table === "vex_config" || table === "vex_session") return q;

    const clientId = global.clientId;
    // Auto-filter by client_id for all operations
    const originalSelect = q.select.bind(q);
    q.select = (...a) => originalSelect(...a).eq("client_id", clientId);

    const originalUpdate = q.update.bind(q);
    q.update = (d) => originalUpdate(d).eq("client_id", clientId);

    const originalDelete = q.delete.bind(q);
    q.delete = () => originalDelete().eq("client_id", clientId);

    const originalInsert = q.insert.bind(q);
    q.insert = (d) => {
        const add = (x) => ({...x, client_id: clientId });
        return originalInsert(Array.isArray(d)? d.map(add) : add(d));
    };

    const originalUpsert = q.upsert.bind(q);
    q.upsert = (d, o) => {
        const add = (x) => ({...x, client_id: clientId });
        return originalUpsert(Array.isArray(d)? d.map(add) : add(d), o);
    };
...
function loadCommands() {
    ...
    fs.readdirSync(pluginPath).filter(f => f.endsWith(".js")).forEach(f => {
        try {
            ...
            commands.set(n, p);
            if (Array.isArray(p.alias)) p.alias.forEach(a => aliases.set(a, n));
        } catch (e) { console.error(`Command ${f} error:`, e.message); }
    });
...
function loadObservers() {
    ...
    fs.readdirSync(observerPath).filter(f => f.endsWith(".js")).forEach(f => {
        ...
        } catch (e) { console.error(`Observer ${f} error:`, e.message); }
    });
    console.log(`Observers loaded: ${observers.length}`);
}

loadCommands(); loadObservers();
try { fs.watch(pluginPath, { persistent: true }, () => loadCommands()); } catch {}
try { fs.watch(observerPath, { persistent: true }, () => loadObservers()); } catch {}
...
async function startBot() {
    ...
    sock.ev.on("connection.update", async ({ connection, lastDisconnect, qr }) => {
        ...
        if (connection === "open") {
            ...
            try { await sock.sendMessage(sock.user.id, { image: { url: BOT_IMAGE }, caption: msg }); } catch {}
        }
    });
...
io.on("connection", (socket) => {
    socket.emit("info", { commands: commands.size, observers: observers.length, prefix: global.prefix });
    socket.on("get_info", () => socket.emit("info", { commands: commands.size, observers: observers.length, prefix: global.prefix }));
    socket.on("get_stats", () => { const mins = Math.floor((Date.now() - START_TIME) / 60000); const h = Math.floor(mins / 60); const m = mins % 60; socket.emit("stats", { uptime: h? `${h}h ${m}m` : `${m}m` }); });
    socket.on("request_pair", async (phone) => {
        ...
        } catch { socket.emit("pairing_code", "ERROR"); }
    });
});

server.listen(PORT, () => { console.log(`VEX running on ${PORT} for ${global.tenantId}`); startBot(); });
