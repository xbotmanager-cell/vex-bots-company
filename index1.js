...
// Set tenant
supabase.rpc("set_tenant", { tenant_name: global.tenantId }).catch(()=>{});
...
supabase.from = (table) => {
    ...
    const ins = q.insert.bind(q); q.insert = d => ins(Array.isArray(d)? d.map(x=>({...x,tenant_id:t})) : {...d,tenant_id:t});
    const up = q.upsert.bind(q); q.upsert = (d,o) => up(Array.isArray(d)? d.map(x=>({...x,tenant_id:t})) : {...d,tenant_id:t}, o);
    return q;
};
...
fs.watch(pluginPath, ()=>loadCommands()); fs.watch(observerPath, ()=>loadObservers());

// ================= SERVER & UI =================
const app = express(); const server = http.createServer(app); const io = new Server(server);
const PORT = process.env.PORT || 10000; const DEPLOY_KEY = process.env.DEPLOY_KEY || "vex2026";

app.use((req,res,next)=>{ if(req.path.startsWith("/socket.io")) return next(); if(req.query.k!==DEPLOY_KEY && req.path!=="/blocked") return res.redirect("/blocked"); next(); });
...
async function startBot() {
    ...
    sock = makeWASocket({ version, logger: pino({ level: "silent" }), printQRInTerminal: false, auth: { creds: state.creds, keys: makeCacheableSignalKeyStore(state.keys, pino({ level: "silent" })) }, browser: ["VEX HOST", "Chrome", "120.0"] });
    global.sock = sock;

    sock.ev.on("connection.update", async ({ connection, lastDisconnect, qr }) => {
        if (qr) { const data = await QRCode.toDataURL(qr); io.emit("qr", data); }
        ...
        if (connection === "open") {
            ...
            // Send welcome message with image and stats
...
io.on("connection", socket => {
    socket.on("request_pair", async phone => {
        try {
            ...
            // WhatsApp sends pairing notification automatically
