 ...
 * No Admin Restriction | High-Speed Mention Logic
 */
...
module.exports = {
    ...
    async execute(m, sock, ctx) {
        ...
        // Ensure command is only used in groups
        if (!m.isGroup) return m.reply("❌ This command can only be used within a group.");

        // Fetch Group Data with Fallback
        const groupMetadata = await sock.groupMetadata(m.chat).catch(() => ({}));
