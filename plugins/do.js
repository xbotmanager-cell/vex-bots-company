...
// Physics constants - CORRECTED SYNTAX
math.import({
    ...
    Na: math.unit('6.02214076e23 mol^-1'), // FIXED: was "1/mol"
...
async function handleStatistics(m, sock, query, ui, lang) {
    try {
        ...
        if (cmd.includes('mean')) {
            result = math.mean(data); op = 'Mean';
        } else if (cmd.includes('median')) {
            result = math.median(data); op = 'Median';
        } else if (cmd.includes('std') || cmd.includes('deviation')) {
            result = math.std(data); op = 'Standard Deviation';
        } else if (cmd.includes('var')) {
            result = math.variance(data); op = 'Variance';
        } else if (cmd.includes('sum')) {
            result = math.sum(data); op = 'Sum';
        } else if (cmd.includes('mode')) {
            result = math.mode(data); op = 'Mode';
        } else {
            result = math.mean(data); op = 'Mean';
        }
...
function isPhysicsQuery(q) {
    const physicsKeywords = [
        'force', 'energy', 'velocity', 'acceleration', 'mass', 'weight', 'pressure', 'power',
        'work', 'momentum', 'f=', 'e=', 'v=', 'a=', 'p=', 'w=', 'kg', 'm/s', 'newton',
        'joule', 'watt', 'pascal', 'coulomb', 'volt', 'ohm', 'farad', 'henry', 'tesla',
        'frequency', 'wavelength', 'density'
    ];
...
function detectLanguage(m, text = '') {
    ...
    if (/\b(ya|na|wa|za|ni|kwa|hii|hiyo|vipi|gani|nini|jibu|hesabu|grafu)\b/i.test(content)) {
        return 'sw';
    }
    return 'en';
}
...
async function callGroq(prompt, maxTokens) {
    ...
    const res = await axios.post(
        'https://api.groq.com/openai/v1/chat/completions',
        {
            ...
            messages: [
                { role: 'system', content: 'You are VEX AI, expert mathematician and physicist. Always show steps.' },
...
async function callCerebras(prompt, maxTokens) {
    ...
    const res = await axios.post(
        'https://api.cerebras.ai/v1/chat/completions',
        {
            model: 'llama3.1-8b',
            messages: [
                { role: 'system', content: 'You are VEX AI. Expert in math/physics.' },
                { role: 'user', content: prompt }
            ],
            max_tokens: maxTokens,
            temperature: 0.2
...
async function callSambaNova(prompt, maxTokens) {
    ...
    const res = await axios.post(
        'https://api.sambanova.ai/v1/chat/completions',
        {
            ...
            messages: [
                { role: 'system', content: 'You are VEX AI.' },
                { role: 'user', content: prompt }
            ],
            max_tokens: maxTokens
        },
