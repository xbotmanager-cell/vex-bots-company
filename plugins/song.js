    ...
    async execute(m, sock, { args, userSettings, prefix }) {
        ...
        const modes = {
            harsh: {
                ...
                selectType: `𝕹𝖔𝖜 𝖈𝖍𝖔𝖘𝖊 𝖙𝖍𝖊 𝖋𝖔𝖗𝖒𝖆𝖙:\n1. 𝕬𝖚𝖉𝖎𝖔 (𝖒𝖕3)\n2. 𝖁𝖎𝖉𝖊𝖔 (𝖒𝖕4)\n\nReply with 1 or 2\n_VEX SYSTEM SELECTION_`,
                react: "🦾",
        ...
        try {
            ...
            if (quotedText.includes("VEX SYSTEM SELECTION") &&!isNaN(args[0])) {
                ...
                if (!cached) return m.reply("❌ Session expired. Search again using " + prefix + "song <name>");
                if (![1, 2].includes(selection)) return m.reply("❌ Reply with 1 or 2");

                await sock.sendMessage(m.chat, { react: { text: "⏳", key: m.key } });
                const loadingMsg = await m.reply(current.downloading);

                try {
                    const safeTitle = cached.title.replace(/[\\/:*?"<>|]/g, '').slice(0, 60);
                    ...
                    if (selection === 1) {
                        ...
                        try { await sock.sendMessage(m.chat, { delete: loadingMsg.key }); } catch {}
                    ...
                    // ===== VIDEO =====
                        ...
                        if (stats.size < 10000) throw new Error("Invalid video file");
            ...
            setTimeout(() => searchCache.delete(m.sender), 300000); // 5 min
            ...
            return m.reply("❌ Failed processing request.");
        } finally {
            setTimeout(async () => { await cleanup(); }, 15000);
        }
    }
};
